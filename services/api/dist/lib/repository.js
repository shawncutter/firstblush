...
async function getUserByPredicate(whereClause, values) {
...
export async function getUserByProviderSubject(provider, providerSubject) {
    return getUserByPredicate("u.auth_provider = $1 and u.provider_subject = $2", [provider, providerSubject]);
}
export async function createUser(provider, providerSubject, displayName) {
...
export async function updateUserPrivacy(userId, input) {
...
export async function requestGroupJoin(groupId, userId) {
...
export async function approveGroupJoin(groupId, ownerId, candidateUserId) {
    const group = await getGroupById(groupId);
    if (!group) {
        return "group_not_found";
    }
...
export async function canViewPost(post, actorId) {
...
export async function listFeed(actorId, groupId) {
    const result = await dbQuery(`
      select
        p.id,
...
export async function getPostEngagementCounts(postId) {
    const result = await dbQuery(`
      select
...
export async function getReactionById(reactionId) {
    const result = await dbQuery(`
      select
        id,
        post_id::text as "postId",
...
export async function getReactionChain(postId, actorId) {
    const result = await dbQuery(`
      select
        r.id,
...
export async function toggleLike(postId, userId) {
    const existing = await dbQuery(`select id::text as id from likes where post_id = $1::uuid and user_id = $2::uuid limit 1`, [postId, userId]);
    let liked = false;
    if (existing.rows[0]) {
    ...
    const countResult = await dbQuery(`select count(*)::text as count from likes where post_id = $1::uuid`, [postId]);
    return {
        liked,
...
export async function sharePost(postId, userId) {
    const before = await dbQuery(`select count(*)::text as count from shares where post_id = $1::uuid`, [postId]);
    const insert = await dbQuery(`
      insert into shares(post_id, user_id)
      values ($1::uuid, $2::uuid)
      on conflict (post_id, user_id) do nothing
    `, [postId, userId]);
    const after = await dbQuery(`select count(*)::text as count from shares where post_id = $1::uuid`, [postId]);
    return {
...
export async function upsertSafetyEdge(actorId, targetId, edgeType) {
    await dbQuery(`
...
export async function listSafetyEdges(actorId) {
    const result = await dbQuery(`
      select
        actor_user_id::text as "actorId",
        target_user_id::text as "targetId",
...
export async function isBlockedOrMuted(viewerId, authorId) {
    if (!viewerId)
        return false;
    const result = await dbQuery(`
      select exists(
...
export async function listModerationQueue(status = "all") {
...
export async function listCreatorMetrics(actorId) {
    const result = await dbQuery(`
      with owned_posts as (
...
export async function trackPostView(postId, actorId) {
    await dbQuery(`
...
export async function getMediaById(uploadId, ownerId) {
    const result = await dbQuery(`
      select
        id,
